...
class FlickrClient: NSObject {
    ...
    func downloadImagesForPin(curPin: Pin, completionHandler: @escaping (_ sucess: Bool, _ error: NSError?)-> Void) {
        ...
        print("When call 'downloardImagesForPin', the request url is \(url)")
        taskForGETMethodWithURL(curURL: url, completionHandler: {(JSONResults, error) in
            ...
            // save numPages variable in Pin Object
            performUIUpdatesOnMain {
                print("requesting get key 'numPages' is \(numPages)")
                CoreDataStack.sharedInstance().saveNumPages(numPages: numPages)
            }
...
//                    CoreDataStack.sharedInstance().savePhotoObject(curPhoto: photo)
                    do {
                        let imageData = try Data(contentsOf: URL(string: photoURL)!) as NSData
                        CoreDataStack.sharedInstance().savePhotoObject(photoURL: photoURL, curPin: curPin, data: imageData)
...
//    // After getting totalpages, random pick a page to download photos
